...
const Item = ({
  ...
  const msg = pageType === 'view' ? '' : 'full';
  const statusList = [{ value: 1, label: '启用' }, { value: 0, label: '禁用' }];
  ...
  const fields = () => (
    <>
      <h3>基础信息</h3>
      <Input label="Demo 编码" id="demo_id" rules={['required', 'number']} msg={msg} />
      <Input label="Demo 名称" id="demo_name" rules={['required', 'string']} msg={msg} />
      <Select
        label="类型"
        id="type"
        ...
        search
      />
      <Select label="状态" id="status" rules={['number']} msg={msg} data={statusList} />
