...
const fieldRules = {
  ...
  phone: () => ({ pattern: /^1[3456789]\d{9}$/, whitespace: true, message: `手机号${WRONG_TYPE}` }), // 手机号
  id: () => ({ pattern: /^\d+x?$/i, whitespace: true, message: `身份证${WRONG_TYPE}` }), // 身份证
  fixed2: () => ({
    pattern: /^0\.([1-9]|\d[1-9])$|^[1-9]\d{0,8}\.\d{0,2}$|^[1-9]\d{0,8}$/i,
    whitespace: true,
    message: '大于0且最多保留两位小数',
  }), // 价格
  number3: () => ({ pattern: /^\d{1,3}$/i, whitespace: true, message: '不能输入超过3位' }), // 3位数字
  normal4: () => ({ pattern: /^.{1,4}$/i, whitespace: true, message: '不能输入超过4位' }), // 3位数字
  normal20: () => ({ pattern: /^.{0,20}$/i, whitespace: true, message: '请输入0～20位字符' }),
  normal250: () => ({ pattern: /^.{0,250}$/i, whitespace: true, message: '请输入0～250位字符' }),
  normal90: () => ({ pattern: /^.{0,90}$/i, whitespace: true, message: '请输入0～90位字符' }),
  normal1000: () => ({ pattern: /^.{0,1000}$/i, whitespace: true, message: '请输入1～1000位字符' }),
};

/**
 * createRules - 根据表单域简写 `rules` 属性生成完整验证规则 (配合 AntPlus.Form 组件使用)
 */
const createRules = (rules, label) =>
  rules.map((rule) => {
...
const createField = (field, label, disabledFields, id) => {
  ...
  if (
    ...
    field.type.displayName !== undefined &&
...
class Input extends Ant.Input {
  ...
  renderCounter = (max) => {
    ...
    return (
      <span className={['input-counter', this.overMaxClass].join(' ')}>
        {count} | {max}
      </span>
    );
  };
  ...
  render() {
    const { msg, auto, max, textarea, rows, ...props } = this.props;

    return (
      <div className={[input, this.countClass].join(' ')}>
        {textarea !== true ? (
          <Ant.Input placeholder={msg} autoComplete={auto} onChange={this.onChange} {...props} />
        ) : (
          <Ant.Input.TextArea
            placeholder={msg}
        ...
        {max !== undefined && this.renderCounter(max)}
      </div>
    );
  }
}
...
class Select extends Ant.Select {
  ...
  render() {
    const { data = [], keys = [], msg, search, clear, ...props } = this.props;
    const [value = 'id', label = 'value'] = keys;
    return (
      <Ant.Select {...this.searchProps} placeholder={msg} allowClear={clear} {...props}>
        {data.map((item) => (
